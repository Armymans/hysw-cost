...
public interface ProjectMapper extends Mapper<BaseProject> {
    ...
    @Select(
            "select " +
                    ...
                    "(select count(*) num from base_project s1 , track_audit_info s2 where s1.id = s2.base_project_id and s2.outsource = '0' and s1.del_flag = '0' and s2.status = '0' AND (s1.district = #{district} or #{district} = '') AND s1.create_time >= #{startTime} AND (s1.create_time <= #{endTime} or #{endTime} = '')) s4, " +
